...
matrix so we can hand it to `detect_change()`. Because the
reconstruction is going into a within-record contrast, we set
`uncertainty_mode = "within_record"`: under spatial stationarity,
the record-specific `sigma_within` replaces the global posterior
residual SD (manuscript Section 4.5.3).
