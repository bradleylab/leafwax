...
Bayesian inversion of leaf-wax hydrogen isotope ratios (δ²H_(wax)) to
...
(Bradley, prepared for submission to *Communications Earth &
Environment*). It can inspect the 14 frozen hierarchical fits. The
validated Bayesian reconstruction interface currently supports the
...
version. Install from the public source repository; release instructions
and accession identifiers will be added after final manuscript review.
...
inversion refuses this preview tier. Complete frozen posteriors are
required. The chordal-run files are publicly available in
[`bradleylab/leafwax-data`](https://github.com/bradleylab/leafwax-data),
while automatic download wiring remains disabled until the validated
release.
...
lists the 14 frozen calibration variants; this does not mean that all 14
have a complete reconstruction design.
...
[`vignette("paleo-record-workflow", package = "leafwax")`](https://bradleylab.github.io/leafwax/dev/articles/paleo-record-workflow.md).
...
chordal fits include an elevation spline and the frozen deposits retain
its coefficients. Elevation is nevertheless not consumed by the current
Bayesian reconstruction designs because the complete new-site multiscale
spline basis is unavailable. This is a reconstruction-interface
boundary, not a claim that elevation was absent from the fitted
calibration.
...
|----|----|
| Methods: Inversion and detection thresholds; Supplement S8.1.1 | [`detect_change()`](https://bradleylab.github.io/leafwax/dev/reference/detect_change.md) |
| Supplement S8.2 vegetation-only envelope | [`compute_vegetation_envelope()`](https://bradleylab.github.io/leafwax/dev/reference/compute_vegetation_envelope.md) |
| Supplement S8.1 local slope and Bayesian inversion | [`local_effective_slope()`](https://bradleylab.github.io/leafwax/dev/reference/local_effective_slope.md), [`invert_d2H()`](https://bradleylab.github.io/leafwax/dev/reference/invert_d2h.md) |
...
submitted manuscript does not present that four-level taxonomy.

## Citation

Release-specific citation metadata and persistent identifiers are
pending final validation. Do not cite this working tree as a released
package.
...
MIT. See [LICENSE](https://bradleylab.github.io/leafwax/dev/LICENSE).
